...
if($action == 'show'){  
    ...
    $sql="select * from tblnational where AID is not null ".$a." 
...
if($action == 'save'){
    ...
    $sql = "insert into tblnational  (Name,LoginID) values ('{$category}',{$userid})";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် National အားအသစ်သွင်းသွားသည်။");
        echo 1;
    }else{
        echo 0;
    }
...
if($action == 'editprepare'){
    $aid = $_POST["aid"];
    $sql = "select * from tblnational  where AID=$aid";
            ...
            $out.="<div class='modal-body'>
                ...
                <input type='hidden' id='action' name='action' value='editsave' />                              
                    <div class='form-group'>
                        <label for='usr'> {$lang['nc_name']} :</label>
...
if($action == 'update'){
    ...
    $sql = "update tblnational  set Name='{$category}' where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]."သည် National အား update လုပ်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($action == 'delete'){
    ...
    $aid = $_POST["aid"];
    $sql = "delete from tblnational  where AID=$aid";
    if(mysqli_query($con,$sql)){
        save_log($_SESSION["username"]." သည် National အားဖျက်သွားသည်။");
        echo 1;
    }
    else{
        echo 0;
...
if($_POST["action"] == 'excel'){
    ...
    $result = mysqli_query($con,$sql);
    $out="";
    $fileName = "NationalReports-".date('d-m-Y').".xls";
    if(mysqli_num_rows($result) > 0)
    {
        $out .= '
        ...
        <table >  
            <tr>
                    <td colspan="2" align="center"><h3>National</h3></td>
            </tr>
...
if($_POST["action"] == 'pdf'){
    ...
    if(mysqli_num_rows($result) > 0)
    {
        ...
        $file = 'NationalPDF'.date("d_m_Y").'.pdf';
        $mpdf->output($file,'D');
    }else{
        $out .= '
        <head><meta charset="UTF-8"></head>
